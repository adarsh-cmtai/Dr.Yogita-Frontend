...
function PaymentStatusHandler({
  ...
  }, [searchParams, allEbooks, router, openPaymentFailedModal, openThankYouModal, setIsLoadingStatusCheck, setSelectedEbookForModal]);
...
export default function LibraryClientPage() {
  ...
  const [isLoadingStatusCheck, setIsLoadingStatusCheck] = useState(false);
  const [selectedEbookForModal, setSelectedEbookForModal] = useState<Ebook | null>(null);
  ...
  const [isPaymentPendingModalOpen, setIsPaymentPendingModalOpen] = useState(false);
      ...
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          const fetched: Ebook[] = data.data.map((eb: any) => ({
            ...
            razorpayPaymentLink: eb.razorpayPaymentLink,
          }));
        ...
        <section className="py-16 bg-white">
          <div className="container mx-auto px-4">
            {filteredEbooks.length > 0 ? (<div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
              {filteredEbooks.map((eb, idx) => (<motion.div key={eb._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: (idx % 4) * 0.08 }}><Card className="h-full border-gray-200 hover:shadow-xl focus-within:shadow-xl flex flex-col overflow-hidden rounded-lg group">

                <div className="relative aspect-[3/4] w-full overflow-hidden bg-gray-100">
                  <NextImage
                    src={eb.image || "/placeholder-ebook.png"}
                    alt={eb.title}
                    fill
                    className="object-contain transition-transform duration-500 group-hover:scale-105 p-2 rounded-lg"
                    quality={85}
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw, 25vw"
                    priority={idx < 4}
                    unoptimized={eb.image?.includes("cloudinary") ? true : false}
                  />

                  <div className="absolute top-3 right-3 bg-pink-500 text-white text-xs font-semibold px-2.5 py-1 rounded-md shadow">₹{eb.price}</div>
                </div>

                <CardContent className="p-5 flex-grow flex flex-col"><div className="mb-3 flex flex-wrap gap-2"><span className="bg-pink-100 text-pink-700 text-xs px-2 py-0.5 rounded-full font-medium">{eb.category}</span><span className="bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded-full font-medium">{eb.pages} pages</span></div><h3 className="text-lg font-semibold text-gray-800 mb-1.5 line-clamp-2 group-hover:text-pink-600 transition-colors">{eb.title}</h3><p className="text-gray-600 text-sm mb-5 flex-grow line-clamp-3">{eb.description}</p>
                  <Button
                    onClick={() => handleBuyClick(eb)}
                    className="w-full bg-pink-500 hover:bg-pink-600 text-white mt-auto"
                    disabled={isLoadingEbooks || !eb.razorpayPaymentLink}
